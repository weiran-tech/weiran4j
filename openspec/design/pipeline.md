...
│
├── state/                               # 【现状 + 将来】现实是什么样、还欠着什么,**流程不依赖此目录**
│   ├── README.md                        #   准入标准、维护约定、管理页共用骨架
│   └── bizs/                            #   每张表一份 <table>.md(八段),cross-biz.md 业务口径,artifact.md 架构/技术;
│                                        #   条目文件内 #NN 编号,由 guards/state-waitlist.mjs 校验(流水线自身欠账在 design/README.md 附录)
│
├── schemas/
