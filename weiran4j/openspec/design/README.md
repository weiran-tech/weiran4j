...
| **重写 readme/ 与 reality/** | 说明文档与现状/待办不可移植(它们讲的是本项目的判断与欠账),但目录划分可以照搬 |
...
流水线自身的欠账已迁到 **[`openspec/state/waitlist-workflow.md`](../state/waitlist-workflow.md)**
—— 与业务侧的待办池([`waitlist.md`](../state/waitlist.md),业务 `B-NN` 与技术 `T-NN` 同一份索引表)分开,
统一按 `state/` 的维护约定处理(新增带症状、关闭要回来划掉)。

**说明文档里不留待办清单**:它没人维护,过期后会让人以为某件事还没做。
待办有唯一的家,就是 `state/`。
