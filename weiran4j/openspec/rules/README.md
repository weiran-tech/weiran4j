...
> **本仓库没有 `advisory/components.md`。** 上游 mono4ts 有一份,管的是 React 公共组件清单;
> weiran4j 的前端还在起步,没有值得「先查再造」的存量组件。
> **等真的出现被重复实现过的公共组件时再建这份文件**,不要为了对齐上游而先建一个空壳 ——
> 空清单会让人以为查过了。
...
⓪ 它漏读一次的代价,是否大到不能接受?
   ...
   是 → 项目根 CLAUDE.md 正文,并在本文件登记为「通道 A 例外」
        ...
        统一响应体 code 是字符串 "0"(前端 `code !== 0` 恒真)、密钥不进版本库
